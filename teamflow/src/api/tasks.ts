...
export const getTasks = (projectId: string, filters?: TaskFilters): Promise<TaskListResponse | Task[]> => {
  const params = { projectId, ...filters };
  return client.get<TaskListResponse | Task[]>('/tasks', { params });
};
...
export const searchTasks = (query: string): Promise<SearchTasksResponse | Task[]> =>
  client.get<SearchTasksResponse | Task[]>('/tasks/search', { params: { q: query } });
